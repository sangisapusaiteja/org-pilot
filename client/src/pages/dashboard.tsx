...
import { Building2, Users, Briefcase, CalendarCheck, Clock, TrendingUp } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Area, AreaChart, CartesianGrid, XAxis, Tooltip, ResponsiveContainer, BarChart, Bar, Legend } from "recharts";
          ...
          <CardContent className="pl-2">
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={CHART_DATA}>
                ...
                />
                <Area type="monotone" dataKey="companies" stroke="hsl(var(--primary))" fillOpacity={1} fill="url(#colorCompanies)" strokeWidth={2} />
                <Area type="monotone" dataKey="employees" stroke="hsl(var(--chart-2))" fillOpacity={0} fill="transparent" strokeWidth={2} strokeDasharray="4 4" />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
...
function ManagerDashboard() {
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-heading font-bold tracking-tight">Team Overview</h2>
          ...
          <CardContent>
            <div className="space-y-4">
              {MOCK_EMPLOYEES.map((emp) => (
                <div key={emp.id} className="flex items-center justify-between border-b border-border/40 pb-4 last:border-0 last:pb-0">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-heading font-bold tracking-tight">Good Morning, Alex!</h2>
          <p className="text-muted-foreground">It's {new Date().toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-medium">Current Status</p>
            <p className="text-xs text-muted-foreground">{isClockedIn ? "Working" : "Not working"}</p>
          </div>
          <StatusBadge status={isClockedIn ? "working" : "inactive"} className="h-8 px-4" />
        </div>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <Card className="border-border/60 shadow-md relative overflow-hidden">
          <div className={`absolute top-0 left-0 w-1 h-full ${isClockedIn ? "bg-emerald-500" : "bg-slate-300"}`} />
          <CardContent className="p-8 flex flex-col items-center justify-center space-y-6">
            <div className="relative">
              <div className={`w-48 h-48 rounded-full border-8 flex items-center justify-center transition-colors duration-500 ${isClockedIn ? "border-emerald-100 bg-emerald-50/50" : "border-slate-100 bg-slate-50/50"}`}>
            ...
            <Card className="border-border/60 shadow-sm">
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm font-medium text-muted-foreground">Late Days</span>
                </div>
              ...
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-muted-foreground">09:00 AM - 06:00 PM</span>
                  </div>
                ))}
              </div>
            </CardContent>
